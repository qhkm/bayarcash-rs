...
impl Bayarcash {
    ...
    pub async fn create_payment_intent(&self, request: &PaymentIntentRequest) -> Result<PaymentIntent> {
    ...
    pub async fn get_portals(&self) -> Result<Vec<Portal>> {
        ...
        if let Some(data) = raw.get("data") {
            return serde_json::from_value(data.clone()).map_err(|e| BayarcashError::Other(e.to_string()));
        }
        Ok(vec![])
    }
    ...
    pub async fn get_channels(&self, portal_key: &str) -> Result<Vec<PaymentChannelInfo>> {
        ...
        Ok(portals.into_iter().find(|p| p.portal_key == portal_key).map(|p| p.payment_channels).unwrap_or_default())
    }
    ...
    pub async fn get_all_transactions(&self, params: &TransactionQueryParams) -> Result<PaginatedResponse<Transaction>> {
        self.require_v3("get_all_transactions")?;
        let query = serde_urlencoded::to_string(params).unwrap_or_default();
        let endpoint = if query.is_empty() { "transactions".to_string() } else { format!("transactions?{}", query) };
    ...
    pub async fn get_transactions_by_order(&self, order_number: &str) -> Result<Vec<Transaction>> {
        ...
        let resp: PaginatedResponse<Transaction> = self.http.get(&format!("transactions?order_number={}", urlencoding::encode(order_number))).await?;
        Ok(resp.data)
    }

    pub async fn get_transactions_by_email(&self, email: &str) -> Result<Vec<Transaction>> {
        self.require_v3("get_transactions_by_email")?;
        let resp: PaginatedResponse<Transaction> = self.http.get(&format!("transactions?payer_email={}", urlencoding::encode(email))).await?;
        Ok(resp.data)
    }

    pub async fn get_transactions_by_status(&self, status: &str) -> Result<Vec<Transaction>> {
        self.require_v3("get_transactions_by_status")?;
        let resp: PaginatedResponse<Transaction> = self.http.get(&format!("transactions?status={}", urlencoding::encode(status))).await?;
        Ok(resp.data)
    }

    pub async fn get_transactions_by_channel(&self, channel: PaymentChannel) -> Result<Vec<Transaction>> {
        self.require_v3("get_transactions_by_channel")?;
        let resp: PaginatedResponse<Transaction> = self.http.get(&format!("transactions?payment_channel={}", channel as u8)).await?;
        Ok(resp.data)
    }
    ...
    pub async fn get_transaction_by_reference(&self, reference: &str) -> Result<Option<Transaction>> {
        ...
        let resp: PaginatedResponse<Transaction> = self.http.get(&format!("transactions?exchange_reference_number={}", urlencoding::encode(reference))).await?;
        Ok(resp.data.into_iter().next())
    }

    // ===== FPX Direct Debit =====

    pub async fn create_fpx_direct_debit_enrollment(&self, data: &impl serde::Serialize) -> Result<FpxDirectDebitApplication> {
        self.http.post("mandates", data).await
    }

    pub async fn create_fpx_direct_debit_maintenance(&self, mandate_id: &str, data: &impl serde::Serialize) -> Result<FpxDirectDebitApplication> {
        self.http.put(&format!("mandates/{}", mandate_id), data).await
    }

    pub async fn create_fpx_direct_debit_termination(&self, mandate_id: &str, data: &impl serde::Serialize) -> Result<FpxDirectDebitApplication> {
        self.http.delete(&format!("mandates/{}", mandate_id), data).await
    }

    pub async fn get_fpx_direct_debit_transaction(&self, id: &str) -> Result<Transaction> {
        self.http.get(&format!("mandates/transactions/{}", id)).await
    }
    ...
    pub async fn create_manual_bank_transfer(&self, data: &ManualBankTransferRequest, _allow_redirect: bool) -> Result<serde_json::Value> {
        if data.payment_gateway != 2 {
            return Err(BayarcashError::Other("payment_gateway must be 2 for manual transfers".to_string()));
        }
        let url = format!("{}/manual-bank-transfer", self.config.manual_transfer_base_url());
        ...
        let mut form = reqwest::multipart::Form::new()
            ...
            .text("merchant_bank_account_holder", data.merchant_bank_account_holder.clone())
        ...
        if let Some(ref path) = data.proof_of_payment_path {
            let file_bytes = tokio::fs::read(path).await.map_err(|e| BayarcashError::Other(format!("Failed to read file: {}", e)))?;
            let filename = std::path::Path::new(path).file_name().map(|n| n.to_string_lossy().to_string()).unwrap_or_else(|| "proof.jpg".to_string());
            form = form.part("proof_of_payment", reqwest::multipart::Part::bytes(file_bytes).file_name(filename));
        }

        self.http.post_multipart(&url, form).await
    }

    pub async fn update_manual_bank_transfer_status(&self, ref_no: &str, status: &str, amount: &str) -> Result<serde_json::Value> {
        let url = format!("{}/manual-bank-transfer/update-status", self.config.manual_transfer_base_url());
        #[derive(serde::Serialize)]
        struct UpdateBody { ref_no: String, status: String, amount: String }
        let body = UpdateBody { ref_no: ref_no.to_string(), status: status.to_string(), amount: amount.to_string() };
        self.http.post_absolute(&url, &body).await
    }
}

impl BayarcashBuilder {
    pub fn sandbox(mut self, enabled: bool) -> Self { self.sandbox = enabled; self }
    pub fn api_version(mut self, version: ApiVersion) -> Self { self.api_version = version; self }
